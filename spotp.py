...
def wait_for_downloads(expected_count, timeout=180):
    ...
    while elapsed < timeout:
        ...
        if len(mp3_files) >= expected_count:
            print(colored("Menemukan file mp3:", "cyan", attrs=["bold"]))                                                                                                                    table = [[i+1, f] for i, f in enumerate(mp3_files)]
            print(tabulate(table, headers=["#", "Nama File"], tablefmt="fancy_grid"))
            return mp3_files
        time.sleep(2)
        elapsed += 2                                                                                                                                                                 return sorted([f for f in os.listdir() if f.endswith(".mp3")])
...
def show_menu():
    ...
    print(colored("2. Masukkan link Spotify baru", "cyan"))                                                                                                                          print(colored("3. Simpan lagu dan keluar", "cyan"))
